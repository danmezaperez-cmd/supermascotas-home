...
export function Testimonials() {
  return (
    <section aria-labelledby="resenas-h" className="shell pt-12 md:pt-20">
      <SectionHeader id="resenas-h" eyebrow="4,8 sobre 5 · 1.240 reseñas" titulo="Lo que dicen los clientes" />
      <ul className="rail rail-bleed gap-4 md:grid md:grid-cols-3 md:gap-5 md:overflow-visible md:[margin-inline:0] md:[padding-inline:0]">
        {testimonios.map((t) => (
          <li key={t.id} className="w-[17.5rem] sm:w-[21rem] md:w-auto">
            <figure className="card flex h-full flex-col gap-3.5 p-5 md:p-6">
              ...
              <figcaption className="mt-auto flex items-center gap-2.5 pt-1">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-pill bg-brand-600 text-small font-extrabold text-white">
                  {t.nombre.slice(0, 1)}
                </span>
                <span>
